...
def clean_data(csv_row):
    cleaned = {}
    cleaned['first_name'] = csv_row[1].strip().split(' ')[0]
    cleaned['last_name'] = csv_row[0].strip()
    cleaned['job_title'] = csv_row[2].strip()
    cleaned['full_or_part_time'] = csv_row[4].strip()
    cleaned['department'] = csv_row[3].strip()
...
try:
    ...
    create_employees_table = "create table employees (id serial primary key, first_name varchar(255) not null, last_name varchar(255) not null, job_title varchar(255) not null, full_or_part_time varchar(1) not null, department varchar(255), annual_salary int not null)"
    ...
    with open('Current_Employee_Names__Salaries__and_Position_Titles.csv') as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                print((insert_employee)%(clean_data(", ".join(row).split(','))) )
